...
std::optional<Syntax::ExternalDeclaration> Parser::ParseExternalDeclaration() {
  ...
  if (function) {
    return Syntax::ExternalDeclaration(*curr, std::move(*function));
  }
  mTokCursor = curr;
  if (auto declaration = ParseDeclaration(); declaration) {
    return Syntax::ExternalDeclaration(*curr, std::move(*declaration));
  }
  assert(0);
  return {};
}
...
std::optional<Syntax::Declaration> Parser::ParseDeclaration() {
  ...
  if (declarationSpecifiers.empty()) {
    logErr(mTokCursor->getLine(mSourceInterface), mTokCursor->getColumn(mSourceInterface),
           "Expected declaration specifiers at beginning of declaration");
  }

  if (mTokCursor == mTokEnd || mTokCursor->getTokenKind() == tok::semi) {
    if (mTokCursor == mTokEnd) {
      logErr(mTokCursor->getLine(mSourceInterface), mTokCursor->getColumn(mSourceInterface),"Missing ; at the end of declaration");
    }else {
      ++mTokCursor;
    }
    return Syntax::Declaration(*curr, std::move(declarationSpecifiers), {});
  }
  std::vector<std::pair<std::unique_ptr<Syntax::Declarator>, std::unique_ptr<Syntax::Initializer>>> initDeclarators;
  /// todo
}

//std::optional<FunctionDefinition> Parser::ParseFunctionDefinition() {
//
...
bool Parser::isAssignment(tok::TokenKind type) {
  return type == tok::plus || type == tok::plus_equal || type == tok::minus_equal ||
  type == tok::star_equal || type == tok::slash_equal || type == tok::percent_equal ||
  type == tok::minus_minus || type == tok::greater_greater || type == tok::amp_equal ||
  type == tok::pipe_equal || type == tok::caret_equal;
}
...
void Parser::Scope::addToScope(std::string name) {
  mCurrentScope.back().insert(std::move(name));
}

bool Parser::Scope::isTypedef(const std::string &name) const {
  for (auto& iter : mTypedefs)
  {
    if (iter.count(name))
    {
      return true;
...
bool Parser::Scope::isInScope(const std::string &name) const {
  for (auto &set : mCurrentScope) {
    if (set.count(name)) {
      return true;
    }
  }
